...
$ID_utilisateur_mod = $_GET['ID'];
$ID_role = $_GET['ID_role'];
$role = $_GET['role'];
//---------------------------------------------------------- INFOS DIFFERENCIEES --------------------------------------------------------------------------------------------------

if ($role == 'pilote') {
    //on cherche la promo et le centre
    $sql = "SELECT nom_promo, nom_centre FROM pilote JOIN promo ON pilote.ID_pilote = promo.ID_pilote JOIN centre ON pilote.ID_centre = centre.ID_centre WHERE pilote.ID_pilote = $ID_role";
...
if ($role == 'etudiant') {
    //on cherche la promo et le centre
    $sql = "SELECT nom_promo, nom_centre FROM etudiant JOIN promo ON etudiant.ID_promo = promo.ID_promo JOIN centre ON etudiant.ID_centre = centre.ID_centre WHERE ID_etudiant = $ID_role";
    $result = $conn->query($sql);
    $row = $result->fetch(PDO::FETCH_ASSOC);
    $nom_promo = $row['nom_promo'];
    $smarty->assign('nom_promo', $nom_promo);
...
$sql = "SELECT `ID_reseau`, `lien_facebook`, `lien_indeed`, `lien_linkedin`, `lien_perso` FROM `reseaux` WHERE ID_utilisateur = '$ID_utilisateur_mod'";
$result = $conn->query($sql);
$row = $result->fetch(PDO::FETCH_ASSOC);
